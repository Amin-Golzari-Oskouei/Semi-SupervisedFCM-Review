...
# Weights are uniformly initialized.
    ...
    while 1:
        ...
        Cluster_elem = (1/(1+alpha)) * (((np.transpose(np.tile((1 + alpha * (1 - b)), (k,1)))) / tmp1) + (alpha*(np.transpose(np.tile(b, (k,1))) * f)))
        ...
        for j in range(k):
            ...
            WBETA = np.transpose(z[j, :] ** q)
            WBETA[np.where(np.isinf(WBETA))] = 0
            section = (Cluster_elem[:, j] ** fuzzy_degree) + (alpha * ((Cluster_elem[:, j]  -  (b * f[:, j] ))** fuzzy_degree))
