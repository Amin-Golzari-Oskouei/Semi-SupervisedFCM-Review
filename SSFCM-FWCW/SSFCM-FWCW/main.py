def main(data, center_points, k, p_init, p_max, p_step, t_max, beta_memory, row, fuzzy_degree, col, q, landa, f, alpha, b):
    import numpy as np
    import math
    import sys
...
# Weights are uniformly initialized.
    ...
    while 1:
        ...
        Cluster_elem = np.transpose((1/(1+alpha)) * (((np.transpose(np.tile((1 + alpha - (alpha*b)), (k,1)))) / tmp1)+ (alpha*(np.transpose(np.tile(b, (k,1))) * f)) ))
        ...
        E_w = object_fun(row, col, k, Cluster_elem, landa, center_points, fuzzy_degree, w, z, q, p, data, f, alpha, b)
...
# If empty or singleton clusters are detected after the update.
        ...
        if (math.isnan(E_w) == False) and (math.isnan(E_w_old) == False) and (abs(1 - E_w / E_w_old) < 10**-6 or Iter >= t_max):
            ...
            return np.transpose(Cluster_elem)
        ...
        mf2 = np.transpose((np.transpose(Cluster_elem)-(np.transpose(np.tile(b, (k,1))) * f ))**fuzzy_degree)
        mf = mf1 + (alpha * mf2)
        ...
        for j in range(k):
            distance[j, :, :] = (1 - np.exp((-1 * np.tile(landa, (row, 1))) * ((data - np.tile(center_points[j, :], (row, 1))) ** 2)))
            dwkm[j, :] = np.matmul(((Cluster_elem[j, :] ** fuzzy_degree) + (alpha * ((Cluster_elem[j, :] - (b * f[:, j])) ** fuzzy_degree))) , np.reshape(distance[j, :, :], (row, col)))
        ...
        for j in range(k):
            ...
            WBETA = np.transpose(z[j, :] ** q)
            WBETA[np.where(np.isinf(WBETA))] = 0
            dw[j] = np.matmul(np.matmul(np.transpose(WBETA), np.transpose(np.reshape(distance[j, :, :], (row, col)))), np.transpose(((Cluster_elem[j, :] ** fuzzy_degree) + (alpha * ((Cluster_elem[j, :] - (b * f[:, j])) ** fuzzy_degree)))))
