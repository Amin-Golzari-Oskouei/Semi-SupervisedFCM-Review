...
def init_center_points(b, data, K):
    labeled_indices = np.where(b == 1)[0]
    labeled_clusters = np.argmax(f[labeled_indices], axis=1)
    S_h_indices = [labeled_indices[labeled_clusters == k] for k in range(K)]
    center_points = np.array([np.mean(data[indices], axis=0) for indices in S_h_indices if len(indices) > 0])
...
for repeat in range(Restarts):
    ...
    if labeled_rate==0:
        ...
        center_points = init_center_points(b, data, k)
