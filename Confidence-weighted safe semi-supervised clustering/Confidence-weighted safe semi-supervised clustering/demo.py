...
import numpy as np
from compute_w_rn import compute_wrn
from main import main
from calculateMetrics import calculateMetrics
import scipy.io
from parameters import parameters
import time
from FCM_main import FCM_main
from compute_sn import compute_sn
...
k, t_max, Restarts, fuzzy_degree, alpha, alpha2, f, labeled_rate, p, sigma = parameters(dataset, lable_true)
...
for repeat in range(Restarts):
    ...
    pre_elements = FCM_main(data, center_points, k, t_max, row, fuzzy_degree, col)
    labeled_mask = b == 1
    unlabeled_mask = b == 0
    w_rn = compute_wrn(data, pre_elements, b, f, p, row, labeled_mask, unlabeled_mask, sigma)
    sn = compute_sn(pre_elements, lable_true, labeled_mask, unlabeled_mask)
    Cluster_elem = main(data, center_points, k, t_max, row, fuzzy_degree, col, f, b, alpha, alpha2, pre_elements, w_rn, labeled_mask, unlabeled_mask, sn)
    end_time = time.time()
