def main(data, center_points, k, t_max, row, fuzzy_degree, col, f, b, alpha, alpha2, pre_cluster_element, w_rn, labeled_mask, unlabeled_mask, sn):
    import numpy as np
    import math
    import sys
...
#-----------------------------------------------------------------------------------------
    ...
    while 1:
        ...
        for i in range(k): 
            tmp1[i, :] = (1/sn * alpha2 * (np.dot(w_rn.T, u_rk[i, :])))
            q_nk[i, :] = (dNK[labeled_mask, i])    +    (sn * alpha * dNK[labeled_mask, i])    +    1/sn * alpha2 * (np.sum(w_rn, axis=0))
            s_rk[i, :] =  alpha2 * np.dot(w_rn/sn, u_nk[i, :])
            t_rk[i, :] = (dNK[unlabeled_mask, i])    +     alpha2 * (np.sum(w_rn/sn, axis=1))

        p_nk = alpha * sn * (f[labeled_mask,:]*dNK[labeled_mask,:]).T  +   tmp1
        ...
        E_w = object_fun(row, col, k, Cluster_elem, center_points, fuzzy_degree, data, f, b, alpha, alpha2, pre_cluster_element, w_rn, u_nk, u_rk,labeled_mask , unlabeled_mask, sn)
        ...
        mf2 = alpha * sn * (u_nk - f[labeled_mask,:].T) ** fuzzy_degree
        for j in range(k):
