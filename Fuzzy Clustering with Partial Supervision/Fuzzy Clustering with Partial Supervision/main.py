def main(data, center_points, k, t_max, row, fuzzy_degree, col, f, b, alpha):
    import numpy as np
    import math
    import sys
...
#-----------------------------------------------------------------------------------------
    ...
    while 1:
        ...
        Cluster_elem = (1/(1+alpha)) * (((np.transpose(np.tile( (1 + alpha - (alpha*b)), (k,1)))) / tmp1) + (alpha*(np.transpose(np.tile(b, (k,1))) * f)) ) 
        ...
        E_w = object_fun(row, col, k, Cluster_elem, center_points, fuzzy_degree, data, f, b, alpha)
        ...
        mf2 = (Cluster_elem -   np.transpose(np.transpose(np.tile(b, (k,1))) * f ) ) ** fuzzy_degree
        mf = mf1 + (alpha * mf2);
