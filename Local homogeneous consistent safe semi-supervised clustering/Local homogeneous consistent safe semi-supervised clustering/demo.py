...
k, t_max, Restarts, fuzzy_degree, alpha, alpha2, f, labeled_rate, p = parameters(dataset, lable_true)
...
for repeat in range(Restarts):
    ...
    labeled_mask = b == 1
    unlabeled_mask = b == 0
    w_rn = compute_wrn(data, pre_elements, b, f, p, row, labeled_mask, unlabeled_mask)
    Cluster_elem = main(data, center_points, k, t_max, row, fuzzy_degree, col, f, b, alpha, alpha2, pre_elements, w_rn, labeled_mask, unlabeled_mask)
    end_time = time.time()
